...
from train_my_dyprag import ParameterTranslator, CrossAttentionParameterTranslator,CrossAttentionHyperNetworkParameterTranslator
def inference_on_jsondataset_with_mydyprag_masking(
    ...
    llm_model = AutoModelForCausalLM.from_pretrained(llm_model_path, device_map=device, torch_dtype=torch.float16)
    ...
    # 初次 token 化比较长度
    teacher_texts = [tokenizer.apply_chat_template(x, tokenize=False).removesuffix("<|eot_id|>") for x in teacher_inputs]
    student_texts = [tokenizer.apply_chat_template(x, tokenize=False).removesuffix("<|eot_id|>") for x in student_inputs]
    ...
    prompts = [tokenizer.apply_chat_template(x, tokenize=False).removesuffix("<|eot_id|>") for x in student_inputs]
...
def inference_on_jsondataset_with_mydyprag_nomask(
    input_json_path: str,
    output_json_path: str,
    llm_model_path: str = "./models/Llama-3.2-1B-Instruct",
    translator_type: str = "parameter-translator",
    ...
    prompts = [tokenizer.apply_chat_template(x, tokenize=False, add_special_tokens=False).removesuffix("<|eot_id|>") for x in student_inputs]
...
def inference_single_sample(
    ...
    # 初次编码
    teacher_prompt = tokenizer.apply_chat_template(teacher_input, tokenize=False).removesuffix("<|eot_id|>")
    student_prompt = tokenizer.apply_chat_template(student_input, tokenize=False).removesuffix("<|eot_id|>")
...
def inference_on_jsondataset_with_rag(
    ...
    tokenizer = AutoTokenizer.from_pretrained(llm_model_path, padding_side="left")
    tokenizer.pad_token_id = tokenizer.eos_token_id
    model = AutoModelForCausalLM.from_pretrained(llm_model_path, device_map=device, torch_dtype=torch.float16)
    model.eval()
    ...
    for item in data:
        ...
        assistant_msg = {"role": "assistant", "content": "The answer is "}
        prompt = tokenizer.apply_chat_template([user_msg, assistant_msg], tokenize=False).removesuffix("<|eot_id|>")
...
if __name__ == '__main__':
    ...
    '''
    input_json_paths=[
        "./data_dev_inference/2wikimultihopqa/comparison_dev.json",
        "./data_dev_inference/2wikimultihopqa/bridge_comparison_dev.json",
        "./data_dev_inference/2wikimultihopqa/inference_dev.json",
        "./data_dev_inference/2wikimultihopqa/compositional_dev.json",
        "./data_dev_inference/hotpotqa/bridge_dev.json",
        "./data_dev_inference/hotpotqa/comparison_dev.json",
        "./data_dev_inference/popqa/total_dev.json",
        "./data_dev_inference/complexwebquestions/total_dev.json",
        "./data_dev_inference/iirc/total_dev.json",
        "./data_dev_inference/strategyqa/total_dev.json",
        "./data_dev_inference/ragtruth/total_dev.json",
    ...
    device = "cuda:6"
    translator_type = "cross-attention-hyper-parameter-translator"#"parameter-translator","cross-attention-parameter-translator"
    translator_path = "./models/Llama-3.2-1B-Instruct-Doc_mask-longt5_cahypt_7/translator_step_72270.safetensors"
    # translator_path="./models/Llama-3.2-1B-Instruct-longt5_capt_14/translator_step_72270.safetensors"
    for input_json_path in input_json_paths:
        if "ragtruth" in input_json_path:
            max_new_tokens = 256
        else:
            max_new_tokens = 128
        inference_on_jsondataset_with_mydyprag_masking(
            input_json_path, 
            input_json_path, 
            llm_model_path, 
            translator_type,
        ...
        # answer_field = f"answer_{Path(llm_model_path).parts[-1]}"
