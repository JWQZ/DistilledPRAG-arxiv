...
class DyPRAGTrainer:
    ...
    def _get_logits_label_mask(self, input_ids, pad_token_id=None):
        ...
        if "<|start_header_id|>assistant<|end_header_id|>" not in chat_template:
            start_tokens = self.llm_tokenizer("[/INST]", add_special_tokens=False)["input_ids"]
...
def get_args():
    ...
    parser.add_argument("--llm_model_path", type=str, default="./models/Mistral-7B-Instruct-v0.2-Doc_mask")
    ...
    parser.add_argument("--translator_type", type=str, default="cross-attention-parameter-translator-s")
    ...
    parser.add_argument("--device_translator", type=str, default="cuda:0")
    parser.add_argument("--device_embedding", type=str, default="cuda:0")
    parser.add_argument("--device_llm", type=str, default="cuda:0")
    parser.add_argument("--train_token", type=int, default=0)
    return parser.parse_args()
if __name__ == "__main__":

    while True:
        if not psutil.pid_exists(1334313):
            break
        time.sleep(1)     
    ...
    if 'snowflake' in args.embedding_model_path:
        ...
        # embedding_model = AutoModel.from_pretrained(args.embedding_model_path)
    ...
    # llm_model = AutoModelForCausalLM.from_pretrained(args.llm_model_path,
    #                                                  torch_dtype=torch.bfloat16
    #                                                 )
    ...
    # translator=translator.to(TRAIN_TYPE)
    # print(translator)
    translator.train()

    # accelerator = Accelerator(mixed_precision="bf16")
    # llm_model, embedding_model, translator = accelerator.prepare(llm_model, embedding_model, translator)
